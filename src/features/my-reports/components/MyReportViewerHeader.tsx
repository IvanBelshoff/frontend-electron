...
type MyReportViewerHeaderProps = {
  reportName: string
  reportIcon: string
  isFavorite: boolean
...
export default function MyReportViewerHeader({
  reportName,
  reportIcon,
  isFavorite,
  isTogglingFavorite,
    ...
    <header className="flex flex-wrap items-center justify-between gap-3">
      <div className="flex min-w-0 items-center gap-3">
        <Link
          ...
          aria-label="Voltar para Meus Relatórios"
          title="Voltar para Meus Relatórios"
        >
          <BackIcon />
        </Link>
        ...
        <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-vscode-accent/25 bg-vscode-accent/10 text-vscode-accent">
          <DashboardMaterialIcon name={reportIcon} className="text-xl" filled />
        </span>

        <h1 className="truncate text-lg font-semibold text-vscode-text" title={reportName}>
          {reportName}
        </h1>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        <FavoriteStarButton
          ...
          label={`${isFavorite ? 'Remover' : 'Adicionar'} ${reportName} dos favoritos`}
