...
type MyDashboardsFiltersDialogProps = {
  isOpen: boolean
  appliedFilters: MyDashboardFilters
  draftFilters: MyDashboardFilters
  onDraftChange: (filters: MyDashboardFilters) => void
  onApply: (filters: MyDashboardFilters) => void
  onClose: () => void
}
...
export default function MyDashboardsFiltersDialog({
  isOpen,
  appliedFilters,
  draftFilters,
  onDraftChange,
  onApply,
  onClose,
}: MyDashboardsFiltersDialogProps) {
  const canClearAppliedFilters = !areMyDashboardFiltersEqual(
    appliedFilters,
    DEFAULT_MY_DASHBOARD_FILTERS,
  )
        ...
        <div className="flex flex-wrap items-center justify-end gap-2 border-t border-vscode-border pt-4">
          {canClearAppliedFilters && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => onApply(DEFAULT_MY_DASHBOARD_FILTERS)}
            >
              Limpar filtros
            </Button>
          )}
