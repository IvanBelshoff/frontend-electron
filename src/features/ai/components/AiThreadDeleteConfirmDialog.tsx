...
type AiThreadDeleteConfirmDialogProps = {
  isOpen: boolean
  threadTitle: string
  isDeleting: boolean
  error?: string | null
...
export default function AiThreadDeleteConfirmDialog({
  isOpen,
  threadTitle,
  isDeleting,
  error,
  onConfirm,
  onCancel,
}: AiThreadDeleteConfirmDialogProps) {
  const displayTitle = threadTitle.trim() || 'Nova conversa'
    ...
    >
      <div className="space-y-4">
        <p className="text-sm text-vscode-text">
          Confirma a exclusão da conversa <strong>{displayTitle}</strong>?
        </p>
